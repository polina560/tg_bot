...
use common\components\{exceptions\ModelSaveException, helpers\ModuleHelper, helpers\UserFileHelper};
...
class ExportImportBehavior extends Behavior
{
    ...
    final public function init(): void
    {
        ...
        if (ModuleHelper::isConsoleModule()) {
            $this->_domain = Yii::$app->environment->APP_DOMAIN;
        } else {
    ...
    final public function export(): ?array
    {
        ...
        $mainItem = $class::find()->where($model->primaryKey)->asArray()->one();
        foreach ($model->attributes() as $attribute) {
            if (!in_array($attribute, $this->exceptFields)) {
        ...
        foreach ($this->imageLinkFields as $imageLinkField) {
            if (!empty($mainItem[$imageLinkField])) {
