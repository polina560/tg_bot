...
class CKFinderInputFile extends InputWidget
{
    ...
    public function init(): void
    {
        ...
        if ($this->hasModel()) {
            ...
            if (is_string($file)) {
                if (file_exists($htdocs . urldecode($file)) && !is_dir($htdocs . urldecode($file))) {
                    $this->defaultValue = $file;
                    $previewUrl = $file . '?_t=' . filectime($htdocs . urldecode($file));
                } elseif (preg_match('/^https?:\/\//', $file)) {
                    $previewUrl = $file;
                }
                if (
                    !empty(Yii::$app->environment->BASE_URI)
                    && !str_starts_with($previewUrl, Yii::$app->environment->BASE_URI)
                ) {
                    $previewUrl = Yii::$app->environment->BASE_URI . $previewUrl;
                }
            }
