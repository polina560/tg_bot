...
final class UserAdminController extends AdminController
{
    ...
    public function behaviors(): array
    {
        return ArrayHelper::merge(parent::behaviors(), [
            'verbs' => [
                ...
                'actions' => ['delete' => ['POST'], 'disable-two-fa' => ['POST']],
            ]
        ]);
    }
     ...
     * @throws \Exception
     */
    public function actionEnableTwoFa(int $id): Response|string
    {
        $model = new TwoFaForm();
        ...
        if ($user->hasTwoFaEnabled()) {
            Yii::$app->session->setFlash('error', Yii::t('app', 'Two-Factor authentication is already enabled.'));
     ...
     * @throws Exception
     */
    public function actionDisableTwoFa(int $id): Response
    {
        $user = $this->findModel($id);
        if ($user->id !== Yii::$app->user->id) {
            throw new ForbiddenHttpException('You are not allowed to update this user.');
        ...
        if (!$user->hasTwoFaEnabled()) {
            Yii::$app->session->setFlash('error', Yii::t('app', 'Two-Factor authentication is not enabled.'));
        } else {
            $user->disableTwoFa();
        }
