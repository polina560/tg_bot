...
class BackendFactory
{
    ...
    protected function registerDefaultAdapters(): void
    {
        $this->registerAdapter(
            'local',
            fn($backendConfig) => $this->createBackend(
                $backendConfig,
                new LocalFilesystemAdapter(
                    $backendConfig['root'],
                    PortableVisibilityConverter::fromArray([
                        'file' => [
                            Visibility::PUBLIC => $backendConfig['chmodFiles'],
                            Visibility::PRIVATE => 0604,
                        ],
                        'dir' => [
                            Visibility::PUBLIC => $backendConfig['chmodFolders'],
                            Visibility::PRIVATE => 0764,
                        ],
                    ], Visibility::PUBLIC)
                ),
            ),
        ...
        $this->registerAdapter('ftp', function ($backendConfig) {
            ...
            return $this->createBackend($backendConfig, new FtpAdapter(FtpConnectionOptions::fromArray($config)));
        });
        ...
        $this->registerAdapter('s3', function ($backendConfig) {
            ...
            $prefix = isset($backendConfig['root']) ? trim($backendConfig['root'], '/ ') : '';

            return $this->createBackend(
                $backendConfig,
                new AwsS3V3Adapter($client, $backendConfig['bucket'], $prefix),
                $filesystemConfig,
            );
        });
    }
