...
class ResizedImageRepository
{
    ...
    public function deleteResizedImages(
        ...
        if ($backend->hasDirectory($resizedImagesPath)) {
            $backend->deleteDir($resizedImagesPath);
        }
    }
    ...
    public function copyResizedImages(
        ...
        if ($sourceBackend->hasDirectory($sourceResizedImagesPath)) {
            ...
            foreach ($resizedImages as $resizedImage) {
                ...
                $sourceImageSize = ResizedImage::getSizeFromFilename(basename($resizedImage['path']));
    ...
    public function renameResizedImages(
        ...
        if ($backend->hasDirectory($resizedImagesPath)) {
            $backend->rename($resizedImagesPath, $newResizedImagesPath);
    ...
    public function getResizedImagesList(
        ...
        foreach ($resizedImagesFiles as $resizedImage) {
            $size = ResizedImage::getSizeFromFilename(basename($resizedImage['path']));
    ...
    protected function createNodeValue($resizedImage): array|string
    {
        if (isset($resizedImage['url'])) {
            return [
                'name' => basename($resizedImage['path']),
                'url' => $resizedImage['url'],
            ];
        }

        return basename($resizedImage['path']);
    }
    ...
    public function getResizedImageBySize(
        ...
        foreach ($resizedImagesFiles as $resizedImage) {
            $resizedImageSize = ResizedImage::getSizeFromFilename(basename($resizedImage['path']));
