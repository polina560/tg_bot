  ...
  tablesAction(async tables => {
    ...
    for (const i in tables) {
      ...
      await axios
          .post(
              props.urlImport,
              {date: dateStr, table, is_file: false},
    ...
    // отправляем запрос на импорт файлов
    axios
        .post(
            props.urlImport,
            {date: dateStr, is_file: true},
            {timeout: 999999, headers: {'Content-Type': 'multipart/form-data'}}
        )
        .then(response => {
          if (response.data.messages) addLog(response.data.messages)
          if (isTrue(dbImportAll.value)) {
            addLogStr('Импорт ВСЕХ файлов ЗАВЕРШЁН', 'success')
...
function exportAction() {
  ...
  tablesAction(async tables => {
    ...
    // отправляем запрос на Экспорт файлов
    axios
        .post(
            props.urlExport,
            {date: dateStr, is_file: true},
        ...
        .then(response => {
          ...
          addLogStr('Экспорт ВСЕХ файлов ЗАВЕРШЁН', 'success')
          allDisable.value = false
...
function downloadAction() {
  ...
  const dateStr = currentBackupsList.value[selectedBackup.value]
  axios
      .get(`${props.urlDownload}?date=${dateStr}`)
      .then(response => {
        if (response.data.messages) addLog(response.data.messages)
        else window.location.href = `${props.urlDownload}?date=${dateStr}`
      })
}
...
function removeAllAction() {
  if (!confirm('Вы действительно хотите удалить все резервные копии БД?')) return
