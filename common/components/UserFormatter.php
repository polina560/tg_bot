...
class UserFormatter extends Formatter
{
    ...
    final public function asVideo(?string $value, array $options = []): string
    {
        ...
        if (str_contains($value, 'youtube.com')) {
            parse_str((string)parse_url($value, PHP_URL_QUERY), $urlParams);
        ...
        if (!empty($youtubeId)) {
            static $id = 0;
            $domain = ModuleHelper::isConsoleModule()
                ? Yii::$app->environment->APP_DOMAIN
    ...
    public function asHtml($value, $config = null): ?string
    {
        if ($value) {
            $domain = ModuleHelper::isConsoleModule()
                ? Yii::$app->environment->APP_DOMAIN
                : Yii::$app->request->hostInfo;
            $value = preg_replace('/<img(.+?)src="((?!(https?:\/\/)).+?)"/', "<img\$1src=\"$domain\$2\"", $value);
        }
        if (!$config) {
            $config = static function (HTMLPurifier_Config $config) {
                ...
                $config->set(
                    'HTML.Allowed',
                    'div[class|style|data-oembed-url],a[href],blockquote,br,del,em,figcaption,figure[class],oembed[url],h1,h2,h3,h4,h5,h6,img[title|alt|src],li,ol,p,pre,i,strong,ul,iframe[src|allowfullscreen|style|frameborder|allow]',
                );
