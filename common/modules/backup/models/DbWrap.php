...
use yii\helpers\{ArrayHelper, BaseConsole, FileHelper};
use ZipArchive;
 ...
 */
class DbWrap extends BaseObject
{
...
SQL
     ...
     * @throws \Exception
     */
    public static function exportFiles(string $dateString): bool
    {
        if (!$dateString) {
            return false;
        }
        $file = self::archive($dateString);
        if (!file_exists($concurrentDirectory = dirname($file))) {
     ...
     * @throws \Exception
     */
    private static function archive(string $data): string
    {
        $zip = new ZipArchive();

        $dbRemote = Yii::$app->getDb()->master;
        $dbName = self::getDsnAttribute('dbname', $dbRemote->dsn);

        if (!is_dir(Yii::getAlias(self::RUNTIME_DIR . self::$dirBackUp) . "/$dbName-$data")) {
            return false;
        }

        $result = $zip->open(
            Yii::getAlias(self::RUNTIME_DIR . self::$dirBackUp) . "/$dbName-$data/uploads.zip",
            ZipArchive::CREATE
        );

        if ($result !== true) {
            throw new \Exception("Failed to create zip archive, error code: " . $result);
        }

        $zip = self::addFileRecursion($zip, Yii::getAlias('@htdocs/uploads'));

        $zip->close();

        return Yii::getAlias(self::RUNTIME_DIR . self::$dirBackUp) . "/$dbName-$data/uploads.zip";
    }


    private static function addFileRecursion(ZipArchive $zip, $dir, $start = ''): ZipArchive
    {
        if (empty($start)) {
            $start = $dir;
        }
        ...
        if ($objs = glob($dir . '/*')) {
            foreach ($objs as $obj) {
                if (is_dir($obj)) {
                    self::addFileRecursion($zip, $obj, $start);
                } else {
...
SQL;
        ...
        if (self::hasS3Storage()) {
            try {
                self::getS3Client()->upload(
                    $ENV->S3_PRIVATE_BUCKET,
                    "$dbName-$dateString/$table.sql",
                    fopen($file, 'r')
                );
            } catch (\Exception $exception) {
                Yii::error($exception->getMessage(), __METHOD__);
     ...
     * @throws Exception
     */
    public static function importFiles(string $table, string $dateString = null): bool
    {
        if (!$table) {
            return false;
        }
        ...
        if (true === $zip->open($file)) {
            $zip->extractTo(Yii::getAlias('@htdocs'));
            $zip->close();
    ...
    public static function remove(string $dateString): bool
    {
        ...
        shell_exec($command);
        if (self::hasS3Storage()) {
            self::getS3Client()->deleteMatchingObjects(
