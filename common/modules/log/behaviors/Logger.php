...
class Logger extends Behavior
{
    ...
    private function rememberDiff(): void
    {
        ...
        // Обрабатываем связанные модели
        foreach ($this->relations as $relation) {
            /** @var AppActiveRecord[]|AppActiveRecord $related */
            $related = $this->owner->{$relation};
            if (is_array($related)) {
                $before = $after = [];
                foreach ($related as $model) {
                    if (!empty($beforeDiff = array_diff_assoc($model->oldAttributes ?? [], $model->attributes))) {
                        $before[] = $beforeDiff;
                    }
                    if (!empty($afterDiff = array_diff_assoc($model->attributes, $model->oldAttributes ?? []))) {
                        $after[] = $afterDiff;
                    }
                }
            } else {
                $before = array_diff_assoc($related->oldAttributes ?? [], $related->attributes);
                $after = array_diff_assoc($related->attributes, $related->oldAttributes ?? []);
            }
            if (!empty($before)) {
                $this->before[$relation] = $before;
            }
            if (!empty($after)) {
                $this->after[$relation] = $after;
            }
        }
    }
