 ...
 * @property string|null      $auth_key             [varchar(32)] Ключ авторизации
 *
 * @property-read array       $profile
 *
 * @property-read UserExt     $userExt
 * @property-read UserExt     $masterUserExt
 * @property-read Email|null  $email
 * @property-read Auth[]      $auths
...
{
    use Identity;
//    Если нужны сессии на разных устройствах
//    use  UserAgentIdentity {
//        UserAgentIdentity::getAuthKey insteadof Identity;
//        UserAgentIdentity::findIdentityByAccessToken insteadof Identity;
//        UserAgentIdentity::generateAuthKey insteadof Identity;
//        UserAgentIdentity::logout insteadof Identity;
//    }
    ...
    private ?UserExt $_userExt;

    final public function getMasterUserExt(): UserExt
    {
        if (!isset($this->_userExt)) {
            $this->_userExt = $this->getUserExt()->one(Yii::$app->db->master);
            if (!is_null($this->_userExt)) {
                $this->populateRelation('userExt', $this->_userExt);
            }
        }
        return $this->_userExt;
    }

    final public function getAuths(): ActiveQuery
    {
