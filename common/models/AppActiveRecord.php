...
abstract class AppActiveRecord extends ActiveRecord
{
    ...
    private bool $extFieldUpdated = false;
    ...
    public function __set($name, $value)
    {
        ...
        if (in_array($name, $this->attributes(), true) && str_contains($name, '.')) {
            $data = explode('.', $name);
            $method = 'get' . ucfirst($data[0]);
            if (
                method_exists($this, $method)
                && $this->{$method}() instanceof ActiveQuery
                && !is_null($this->{$data[0]})
            ) {
                $this->{$data[0]}->{$data[1]} = $value;
                $this->extFieldUpdated = true;
            }
        }
    }
    ...
    {
        if ($this->extFieldUpdated) {
            $models = [];
            foreach (static::externalAttributes() as $externalAttribute) {
                [$model] = explode('.', $externalAttribute);
